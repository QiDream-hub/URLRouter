...
static void free_node(route_node_t *node) {
    ...
    if (node->segment) free(node->segment);
    free(node);
}
...
static int is_keyword(const char *segment) {
    if (!segment || strlen(segment) < 4) return 0;
...
static char* extract_keyword(const char *segment) {
    if (!is_keyword(segment)) return NULL;
...
// 查找子节点
static route_node_t* find_child(route_node_t *node, const char *segment, int prefer_keyword) {
...
// 注册路由的内部实现
static int register_route(router_t *router, 
                          http_method_t method,
                          char **segments,
                          int segment_count,
                          route_callback_t callback,
                          void *cbdata) {
    if (method >= HTTP_METHOD_COUNT) return -1;
...
int router_register(router_t *router, 
                    ...
                    route_callback_t callback,
                    void *cbdata) {
    if (!router || !pattern || !callback) return -1;
    if (method >= HTTP_METHOD_COUNT) return -1;
    ...
    int result = register_route(router, method, segments, segment_count, 
                                 callback, cbdata);
...
route_callback_t router_match(router_t *router,
    ...
    if (!router || !url || !out_params) return NULL;
    ...
    if (param_count > 0) {
        ...
        if (out_params->params) {
            ...
            // 分配失败，清理
            for (int i = 0; i < param_count; i++) {
                if (params_tmp[i]) free(params_tmp[i]);
            }
            return NULL;
        }
    }
